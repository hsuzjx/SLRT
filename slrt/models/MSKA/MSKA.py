...
class MSKA(SLRTBaseModel):
    ...
    def forward(self, patches, kps, patch_lgt, kps_lgt) -> Any:
        for i in range(len(patches)):
            assert patch_lgt[i] == kps_lgt[i]
        if self.flag:
            for i in range(patches.shape[2]):
                img = patches[0, 0, i, :, :, :].permute(1, 2, 0).cpu().numpy()
                self.logger.experiment.log({f"patch": wandb.Image(img)})
            self.flag = False

        # 获取输入数据的维度
        N, T, V, C = kps.shape
        patches = patches.view(N, T, V, -1)
        kps = torch.cat([kps, patches], dim=-1)

        N, T, V, C = kps.shape

        # 调整输入数据的维度顺序并保持连续性
        kps = kps.permute(0, 3, 1, 2).contiguous().view(N, C, T, V)
    ...
    def step_forward(self, batch) -> Any:
        ...
        patches, kps, y_glosses, y_translation, patches_lgt, kps_lgt, y_glosses_lgt, y_translation_lgt, name = batch
        batch_size = len(name)

        outputs = self.forward(patches, kps, patches_lgt, kps_lgt)

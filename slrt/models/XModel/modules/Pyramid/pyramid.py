...
class Upsampling(nn.Module):
    def __init__(self, in_channels=832, out_channels=480, kernel_size=3, scale=(2, 2, 2), interpolate=True):
        ...
        if interpolate:
            ...
            self.conv_trans_s = nn.ConvTranspose3d(in_channels, out_channels, kernel_size=(1, kernel_size, kernel_size),
                                                   stride=(1, scale[1], scale[2]), padding=padding_s,
                                                   output_padding=out_padding_s, bias=False)
            ...
            self.conv_trans_t = nn.ConvTranspose3d(out_channels, out_channels, kernel_size=(kernel_size, 1, 1),
                                                   stride=(scale[0], 1, 1), padding=padding_t,
                                                   output_padding=out_padding_t, bias=False)
...
class PyramidNetwork_v2(nn.Module):
    def __init__(self, channels=[832, 480, 192], kernel_size=1, num_levels=3, temp_scale=[2, 1], spat_scale=[2, 2]):
        ...
        for i in range(num_levels - 1):
            self.upsample_layers.append(Upsampling(channels[i], channels[i + 1], kernel_size,
                                                   scale=(temp_scale[i], spat_scale[i], spat_scale[i]),
                                                   interpolate=True))
...
class PyramidNetwork(nn.Module):
    def __init__(self, channels=[832, 480, 192, 64], kernel_size=3, num_levels=4, temp_scale=[2, 1, 1],
                 spat_scale=[2, 2, 2], need_fused=False):
        ...
        for i in range(num_levels - 1):
            self.upsample_layers.append(Upsampling(channels[i], channels[i + 1], kernel_size,
                                                   ...
                                                   interpolate=False))
            if need_fused:
                self.temp_downsample_layers.append(
                    Temporal_Downsampling(channels[num_levels - 1 - i], channels[num_levels - 1 - i], kernel_size,
                                          scale=2))

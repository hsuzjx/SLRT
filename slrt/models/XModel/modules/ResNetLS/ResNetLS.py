...
class ResNetLS(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, num_layers=3):
        self.num_layers = num_layers
        # 初始化输入平面的通道数
        self.inplanes = 64
        # 调用父类的构造函数
        super(ResNetLS, self).__init__()
        # 定义卷积层，用于模型的初始卷积操作
...
class ResNet18LS(ResNetLS):
    def __init__(self, pretrained=False, model_dir='./.models', num_classes=1000, num_layers=3):
        super(ResNet18LS, self).__init__(BasicBlock, [2, 2, 2, 2], num_classes, num_layers=num_layers)
        if pretrained:
...
class ResNet34LS(ResNetLS):
    def __init__(self, pretrained=False, model_dir='./.models', num_classes=1000, num_layers=3):
        super(ResNet34LS, self).__init__(BasicBlock, [3, 4, 6, 3], num_classes, num_layers=num_layers)
        if pretrained:
            model_dir = os.path.abspath(model_dir)
            os.makedirs(model_dir, exist_ok=True)
